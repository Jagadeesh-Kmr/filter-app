...
class AppStore extends Component {
  ...
  getFilteredApps = searchedApps => {
    ...
    const filteredApp = searchedApps.filter(
  ...
  render() {
    ...
    const filteredApps = this.getFilteredApps(searchResult)
    return (
      <>
        <div className="app-container">
          <h1 className="app-h1">App Store</h1>
          <ul className="ul-list">
            {tabsList.map(eachList => (
              <TabDetails
                ...
                isActive={activeTabId === eachList.tabId}
              />
            ))}
          </ul>
          ...
          <ul className="ul-list">
            {filteredApps.map(eachList => (
              <TabItem key={eachList.appId} tabItems={eachList} />
            ))}
          </ul>
        </div>
      </>
